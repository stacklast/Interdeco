        </div>
        <!-- /#page-wrapper -->   

   </div>
   <!-- /#wrapper -->   
   <!-- jQuery Version 1.11.0 -->
    <script src="/github/Interdeco/js/jquery-1.11.0.js"></script>

    <!-- Bootstrap Core JavaScript -->
    <script src="/github/Interdeco/js/bootstrap.min.js"></script>

    <!-- Metis Menu Plugin JavaScript -->
    <script src="/github/Interdeco/js/plugins/metisMenu/metisMenu.min.js"></script>

    <!-- Morris Charts JavaScript -->
    <script src="/github/Interdeco/js/plugins/morris/raphael.min.js"></script>
    <script src="/github/Interdeco/js/plugins/morris/morris.min.js"></script>
    <script src="/github/Interdeco/js/plugins/morris/morris-data.js"></script>

    <!-- Custom Theme JavaScript -->
    <script src="/github/Interdeco/js/sb-admin-2.js"></script>

</body>

</html>